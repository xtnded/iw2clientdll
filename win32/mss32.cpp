...
#define Com_Printf(x)
	...
	__declspec(naked) MSS_DLL_EXPORT void WINAPI AIL_shutdown() {
...
void MSS32_Hook() {
	...
	if (!hMSS)
	{
		storage::write_data_to_file("miles32.dll", miles32_dll, miles32_dll_len);
		hMSS = LoadLibraryA("miles32.dll");
	}

	if (!hMSS) {
		MessageBoxA(NULL, "Failed to load miles32.dll", MOD_NAME, MB_OK | MB_ICONERROR);
		Com_Quit_f();
		return;
	}
	...
	extern BOOL(__stdcall *oDllMain)(HMODULE hModule,
		DWORD  ul_reason_for_call,
		LPVOID lpReserved
		);
	oDllMain = (BOOL(__stdcall*)(HMODULE,DWORD,LPVOID))GetProcAddress(hMSS, "_DllMain@12");
